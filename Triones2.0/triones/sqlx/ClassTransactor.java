...
import triones.bas.ClassFactory;
import triones.bas.ClassesFactory;
import triones.sql.DBMS;
import triones.sql.Syntax;
import triones.sql.Rule;
import triones.sql.WYQRule;
...
<DT><B>对象概述：</B><DD>
<pre>
    ClassTransactor的设计目的在于提供逻辑层到存储层的统一接口标准。在大多数应用开发中，
...
4.一旦数据库系统需要移植到另一种数据库系统中，则很有可能产生SQL语句的不兼容性。
    ...
    采用ClassTransactor可以在以上各个方面带来很大的改善。首先，它规范了逻辑层与存储层进
行数据交换的过程，并且可以保证60％以上的数据库操作过程是完全一致的，因此可以在此基础
上开发代码自动生成工具，从而提高数据交换方面的工作效率。其次，由于ClassTransactor完成数据
库操作的语句是动态生成的，因此当库表结构发生变化时，只需修改字段的映射机制，95％以上
的程序代码不需要修改。再次，采用ClassTransactor方式可以很好地保证在软件设计中即提高了编程的便利
性而又保证体系结构的灵活性和高适用性。最后，ClassTransactor与Transaction、Connector很好地组成了
一整套与数据库系统具体类型略关的数据交换机制，可以方便地实现跨数据库平台的程序开发。

    目前ClassTransactor除了能提供对单个库表的数据操作外，还提供了多表联合查询机制。此外，
...
public class MyExample
    ...
    在使用时先将用于存取数据的对象转换成ClassTransactor语义，然后调用DBTrans的insert、
...
{	Debug.IFDebug = true;
	...
	{ //连接数据库
		...
		//转换成ClassTransactor,指定与MyExample对应的数据库表StreamExample
		//实际开发中，建议通过实现trionesII.sql.Rule将类和属性名与数据库表及字段名以某种规则进行映射
		ClassTransactor com = new ClassTransactor(example,"StreamExample");
...
*@version 2003.03.07
*/
public class ClassTransactor extends Transactor implements Cloneable,Serializable
{	
/**版本号（创建于 2003.03.07）.
<DL>
<DT><B>说明：</B><DD>
...
*/
  private boolean _ZeroAsNull = false;
  

/**
* 组成复合ClassTransactor的对象类型清单（创建于 2003.03.07）.
<DL>
<DT><B>说明：</B><DD>
<pre>
...
*/
	static Rule DEFAULT_RULE = new WYQRule();
...
ClassTransactor com = new ClassTransactor(u);
com.hideCndFields();	
...
<pre>
	...
	ClassTransactor com1 = new ClassTransactor(u);
	ClassTransactor com2 = new ClassTransactor(q);
	
	//确定在FBI_USER中的显示字段
	com1.hideFields();
	TransactorMethod.showFields(com1,"u_email,u_name,u_id");	
	...
	//以u_id为两表关联的关键字
	ClassTransactor com3 = ClassTransactor.join(com1,com2,"u_id","u_id");
...
*/
	...
	{ if(_Factory==null)
		{	if(isMultiple())
				_Factory = new ClassesFactory(getMultiClasses());
			else
				_Factory = new ClassFactory(getSimpleClass());
		}
...
*/
	...
	ClassTransactor com = new ClassTransactor(example);
	...
	@param obj 业务逻辑对象.
	@see #ClassTransactor(Object,Rule)
	@see #ClassTransactor(Object,Rule,boolean)
	*/

	public ClassTransactor(Object obj)
	{	super();
		init(obj.getClass(),DEFAULT_RULE);
	...
	ClassTransactor com = new ClassTransactor(example,rule);
...
* @see #ClassTransactor(Object)
* @see #ClassTransactor(Object,Rule,boolean)
*/
	public ClassTransactor(Object obj,Rule rule)
	{	super();
		init(obj.getClass(),rule);
		addFields(obj,rule);
	}
...
* @param rule 数据库表对象与业务逻辑对象之间的映射规则.
*/
	public ClassTransactor(Class cls,Rule rule)
	{	super();
		init(cls,rule);
		addFields(cls,rule);
	}
...
* @param cls 对象类.
*/
	public ClassTransactor(Class cls)
	{	super();
		init(cls,DEFAULT_RULE);
		addFields(cls,DEFAULT_RULE);
	}	
/**
* 构造方法（创建于 2005.05.31）.
<P><DL>
...
* @param obj 业务逻辑对象.
* @param rule 数据库表对象与业务逻辑对象之间的映射规则.
* @param zeroAsNull true代表对基本类型中的0视为null.
* @see #ClassTransactor(Object)
* @see #ClassTransactor(Object,Rule)
*/

	public ClassTransactor(Object obj,Rule rule,boolean zeroAsNull)
	{	super();
    	_ZeroAsNull = zeroAsNull;
		init(obj.getClass(),rule);
		addFields(obj,rule);
	}
			...
			for(int i=0;i<lFields.length;i++)
				...
				{	ls_fName = lFields[i].getName();
					...
					lo_fVal = lFields[i].get(obj);//maybe throw IllegalAccessException
          			...
          			if(_ZeroAsNull)
          			{ if(DataTypeMethod.isBaseType(lc_fType)&&DataTypeMethod.isZero(lo_fVal)) lo_fVal = null;
          			}
...
* @param rule 命名规则.
*/	
	private void addFields(Class cls,Rule rule)
	{ 	Field[] lFields = _SimpleClass.getFields();	
		String ls_fName,ls_fDBName;
		Class lc_fType;
		...
		for(int i=0;i<lFields.length;i++)
			...
			{	ls_fName = lFields[i].getName();
				ls_fDBName = rule.toFieldDBName(ls_fName);
				lc_fType = lFields[i].getType();
      			addField(ls_fName,ls_fDBName,lc_fType,null);
			}
	}
...
*/
	...
	/**
	 * 创建一个复合类型的ClassTransactor（创建于 2003.03.07）.
	<P><DL>
	<DT><B>说明：</B><DD>
	<pre>
	返回值中的ClassTransactor只继承参数ClassTransactor的条件信息，排序、分组等信息不继承.
	</pre>
	<DT><B>示例：</B><DD>
	<pre>
...
* @return 复合类型的ClassTransactor.
* @exception NoSuchElementException 找不到的列名.
*/
	public static ClassTransactor join(ClassTransactor com1,ClassTransactor com2,String fieldNames1,String fieldNames2)
	{	return join(com1,com2,fieldNames1,fieldNames2," = ");
	}
	
	/**
	 * <img src="new.gif" width="28" height="11" border="0">创建一个复合类型的ClassTransactor（创建于 2003.03.07）.
	<P><DL>
	<DT><B>说明：</B><DD>
	<pre>
	...
	在ORACLE中，分别为" (+)= "和" =(+) ".
	特别需要指出的是：com1和com2中原有的排序、分组等信息将不对新产生的ClassTransactor有效.
	</pre>
	<DT><B>示例：</B><DD>
	<pre>
	略	
...
* @exception NoSuchElementException 找不到的列名.
*/
	private static ClassTransactor join(ClassTransactor com1,ClassTransactor com2,String fieldNames1,String fieldNames2,String op)
  {	String [] ls_FieldNames1 = StringMethod.toChilds(fieldNames1,Syntax.FIELD_APART);
		...
		ClassTransactor com = new ClassTransactor();
		...
		com.setDBName(com1.getDBName() + Syntax.FIELD_APART + com2.getDBName());
		/*
		...
		*/	
		...
		//将条件合并
		com.appendCndTransactor(Syntax.CNDRELATION_AND,com1);
		com.appendCndTransactor(Syntax.CNDRELATION_AND,com2);
...
<pre>
	...
	ClassTransactor com = ClassTransactor.join(com1,com2,"u_id","u_id");
